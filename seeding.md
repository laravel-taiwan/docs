...
> [!NOTE]  
> 在 `run` 方法的簽名中，您可以對所需的任何依賴進行型別提示。它們將通過 Laravel [服務容器](/docs/{{version}}/container) 自動解析。
...
在運行種子時，您可能希望防止模型發送事件。您可以使用 `WithoutModelEvents` 特性來實現這一點。使用 `WithoutModelEvents` 特性時，即使通過 `call` 方法執行其他種子類，也確保不會發送任何模型事件：
...
您也可以使用 `migrate:fresh` 命令結合 `--seed` 選項來填充您的資料庫，這將刪除所有資料表並重新執行所有遷移。這個命令對於完全重建您的資料庫很有用。`--seeder` 選項可用於指定要運行的特定填充器：
...
某些填充操作可能會導致您修改或遺失資料。為了防止您對正式資料庫運行填充命令，您將在 `production` 環境中執行填充器之前被提示確認。要強制執行填充器而不提示，請使用 `--force` 標誌：
